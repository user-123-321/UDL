...
class GCTrainer(BaseTrainer):
    r"""
        ...
        Base:
            `BaseTrainer`
    """
    def __init__(self, model, scenario, optimizer_fn, loss_fn, device, batch_size=128, **kwargs):
    ...
    def prepareLoader(self, curr_dataset, curr_training_states):
        ...
        train_loader = dgl.dataloading.GraphDataLoader(curr_dataset['train'], batch_size=self.batch_size, shuffle=True, drop_last=False, num_workers=0, worker_init_fn=self._dataloader_seed_worker, generator=g_train)
        
        # dataloader for validation dataset
        g_val = torch.Generator()
        g_val.manual_seed(0)
        val_loader = dgl.dataloading.GraphDataLoader(curr_dataset['val'], batch_size=self.batch_size, shuffle=False, drop_last=False, num_workers=0, worker_init_fn=self._dataloader_seed_worker, generator=g_val)
        
        # dataloader for test dataset
        g_test = torch.Generator()
        g_test.manual_seed(0)
        test_loader = dgl.dataloading.GraphDataLoader(curr_dataset['test'], batch_size=self.batch_size, shuffle=False, drop_last=False, num_workers=0, worker_init_fn=self._dataloader_seed_worker, generator=g_test)
