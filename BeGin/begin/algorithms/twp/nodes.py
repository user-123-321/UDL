...
class NCTaskILTWPTrainer(NCMinibatchTrainer):
    ...
    def inference(self, model, _curr_batch, training_states, return_elist=False):
        ...
        preds = model.bforward(blocks, blocks[0].srcdata['feat'], return_elist=return_elist, task_masks=blocks[-1].dstdata['task_specific_mask'])
        if return_elist:
            preds, elist = preds
    ...
    def afterInference(self, results, model, optimizer, _curr_batch, training_states): 
        ...
        return {'loss': cls_loss.item(),
                'acc': self.eval_fn(self.predictionFormat(results), _curr_batch[-1][-1].dstdata['label'])}
...
class NCClassILTWPTrainer(NCTrainer):
    def __init__(self, model, scenario, optimizer_fn, loss_fn, device, **kwargs):
        """
            TWP needs three additional parameters `lambda_l`, `lambda_t`, and `beta`.
            `lambda_l` is the hyperparamter for the regularization term (similar to EWC) used in :func:`afterInference`.
    ...
    def inference(self, model, _curr_batch, training_states, return_elist=False):
        ...
        preds = model(curr_batch.to(self.device), curr_batch.ndata['feat'].to(self.device), return_elist=return_elist)
        if return_elist:
            preds, elist = preds
        preds = preds[mask]
...
class NCClassILTWPMinibatchTrainer(NCMinibatchTrainer):
    def __init__(self, model, scenario, optimizer_fn, loss_fn, device, **kwargs):
        """
            TWP needs three additional parameters `lambda_l`, `lambda_t`, and `beta`.
            `lambda_l` is the hyperparamter for the regularization term (similar to EWC) used in :func:`afterInference`.
    ...
    def inference(self, model, _curr_batch, training_states, return_elist=False):
        ...
        blocks = [b.to(self.device) for b in blocks]
        labels = blocks[-1].dstdata['label']
        preds = model.bforward(blocks, blocks[0].srcdata['feat'], return_elist=return_elist)
        if return_elist:
            preds, elist = preds
        loss = self.loss_fn(preds, labels)
        return {'preds': preds, 'loss': loss, 'elist': elist if return_elist else None}
